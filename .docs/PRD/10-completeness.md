...
finished: every page and every component reads from the catalogue, both catalogues carry the same
909 keys, `tsc` refuses a missing translation, and no `toLocaleDateString('en-GB')` survives
anywhere. What is left is that **the Hindi was written by the same person who wrote the English.**
Register and politeness level are what a school notices and no type checker has an opinion about
them. The row moves to ✅ the day somebody who teaches in Hindi has read `hi.ts` — and that is the
