...
| FR-VER-009 |    P2    | Bulk verification (approve many at once).                                  | **Built.** Multi-select, up to 100 at once. Partial success is reported per request, not rolled back. |

## Access implications (server-enforced)

- Verified **student** → read academics of their one class.
- Verified **parent** → read academics of each verified child's class; act _for the child_ (leave).
